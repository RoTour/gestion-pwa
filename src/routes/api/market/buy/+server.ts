...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	try {
		await MarketRepository().buyResources(user.email ?? '', resource, quantity);
		return json({ message: 'Resources bought' });
	} catch (e) {
		console.error(e);
		return json({ message: 'Error buying resources' }, { status: 500 });
	}
}